...
def send_and_persist_notifications(
    ...
    *,
    ...
    return dispatch_due_notifications(db_path, config, sender=sender, as_of=started_at)
...
def resend_undelivered_notifications(
    ...
    *,
    ...
    if not callable(sender):
        raise TypeError("sender must be callable")
    return dispatch_due_notifications(db_path, config, sender=sender, as_of=as_of)
